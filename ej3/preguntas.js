// Preguntas v2

var my_path = '/home/netman/cursos/node.js/apps/node_modules/';
var express = require(my_path + 'express');
var path = require('path');
var bodyParser = require(my_path + 'body-parser');

var app = express();

// TODO: Set title

app.use(bodyParser.urlencoded({ extended: true }));
app.use(express.static(path.join(__dirname, 'public')));    // TODO: ¿Esto es realmente necesario?

// Transacción 1: carga del formulario
//       -> GET /preguntas

app.get('/preguntas', function(req, res) {
   res.send('<!DOCTYPE html><html lang="en">
			 +   '<head>'
			 +   '<meta charset="UTF-8">'
			 +	  '<title>Un par de preguntas</title>
			 +	  '</head>'
			 +   '<body><h1>Preguntas:</h1>'
          +   '<form method="post" action="/respuesta">'
          +     '<input type="hidden" name="id" value="1" />'
          +     '<label for="p">¿Quién descubrió América?</label></br>'
          +      '<input type="text" name="respuesta" /></br>'
          +      '<input type="submit" value="Enviar" />'
          +   '</form>'
          +   '<form method="post" action="/respuesta">'
          +     '<input type="hidden" name="id" value="2" />'
          +     '<label for="p">¿Capital de Portugal?</label></br>'
          +     '<input type="text" name="respuesta" /></br>'
          +     '<input type="submit" value="Enviar" />'
          +   '</form>'
          + '</body></html>');
});

// Transacción 2: envío y proceso de datos
//          -> GET /respuesta

app.post('/respuesta', function(req, res, next) {
	// Hay que comprobar que las respuestas son correctas o no (usar regexp aquí).
	// Indicar en caso de fallo cuál era la respuesta correcta.
	// Incluir enlace a la página inicial.

	var respuesta_match = { '1': '(Crist[oó]bal )*Col[oó]n', '2': 'Lisboa' };		// Idea: Incluir las respuestas como campo oculto.
	var respuesta_correcta = { '1': 'Colón', '2': 'Lisboa' };

	// Debería comprobar que la variable req.body.respuesta existe antes de usarla.
	// if (req.query.respuesta) {
	// Considerar los casos de error cuando el id de la pregunta no es ni 1 ni 2 o cuando no existe el parámetro respuesta.

	var regexp = new RegExp(respuesta_match[req.body.id], "i");		// Podría usar toLowerCase() en vez de usar las regexps.
   if (req.body.respuesta.match(regexp)) {
		output = '¡Enhorabuena!</br> La respuesta es correcta.</br>';
	} else {
	   output = 'Has fallado.</br> La respuesta correcta era: ' + respuesta_correcta[req.body.id] + '</br>';
	}

	output += '</br> <a href="./preguntas">Volver a la página inicial.</a>';

  	res.send(output);
});

// Tratamiento de errores (por ejemplo, si escrbimos simplemente localhost:8000)

app.get('*', function (req,res) {
	res.send('Operación inválida, agrege /preguntas o /respuesta');
});

// Middleware de error
app.use(function (err,req,res,next) {
	res.send(err.toString()); // Se envia el mensaje de error
});

app.listen(8000);
